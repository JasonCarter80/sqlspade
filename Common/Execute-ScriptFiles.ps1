Function Execute-ScriptFiles
{
	...
	#Load a list of all files from the appropriate scripts folder (\\ray\dba_public$\cur_installs\SqlScripts)
	if ($sequence -eq "pre")
	{
		[array] $files = get-childitem -path $Script:PreScripts
        $count = $files.Count
		$message = "Applying $count Standard Pre-Install Scripts"
	}
	else
	{
		[array] $files = get-childitem -path $Script:PostScripts
		$count = $files.Count
